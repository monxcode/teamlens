...
export function ChatMessage({
  ...
  const handleEditFromContext = (msgId: string) => {
    setIsEditing(true);
  };
  ...
  return (
    <>
      ...
      {/* Lightbox */}
      {lightboxIndex !== null && msg.attachments && (
        <Lightbox
          attachments={msg.attachments}
      ...
      {/* Details Modal */}
      {showDetails && (
        <div className="fixed inset-0 z-[10000] bg-black/50 flex items-center justify-center p-4" onClick={() => setShowDetails(false)}>
          <div className="bg-card rounded-xl border shadow-xl max-w-sm w-full p-5 space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              {msg.editedAt && (
                <div className="flex justify-between">
                  ...
                  <span className="text-sm">{new Date(msg.editedAt).toLocaleString()}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Message ID</span>
                <span className="text-sm font-mono text-xs">{msg.id}</span>
              </div>
              {msg.attachments && msg.attachments.length > 0 && (
                <div>
                  ...
                  <div className="mt-1 space-y-1">
                    {msg.attachments.map((a) => (
                      <div key={a.id} className="flex justify-between text-xs">
                        ...
                        <span className="text-muted-foreground">{(a.fileSize / 1024).toFixed(1)} KB</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <button
              ...
              className="w-full py-2 rounded-lg bg-muted hover:bg-muted/80 text-sm font-medium transition-colors"
            >
              Close
            </button>
          </div>
